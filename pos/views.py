...
def registrar_cliente(data, asesor):
    ...
    # Obtener fecha_agenda solo si la descripción es "AGENDADO"
    descripcion  = data.get("descripcion", "").strip().upper()
    fecha_agenda = data.get("fecha_agenda") if descripcion in ["AGENDADO", "VOLVER A LLAMAR"] else None
    ...
    cliente = Cliente.objects.create(
        ...
        descripcion     = descripcion,  # Convertimos a mayúsculas por consistencia
        ...
        fecha_agenda    = fecha_agenda  # Se agrega aquí la fecha de agenda
    )
    return cliente
...
def dashboard(request):
    ...
    # Obtener el primer y último día del mes actual
        ...
        # Filtrar ventas del asesor autenticado en el mes actual
        ventas_mes = Venta.objects.filter(
            fecha_hora__date__range=(primer_dia_mes, ultimo_dia_mes),
            IdAsesor=asesor_autenticado
        )

        # Agrupar ventas por día
        ventas_por_dia = ventas_mes.annotate(
        ...
        ventas_por_dia_dict = {
            venta['fecha'].strftime('%Y-%m-%d'): venta['count'] for venta in ventas_por_dia
        } if ventas_por_dia else {"Sin datos": 0}

        # Obtener clientes agendados del asesor autenticado
        subquery = Cliente.objects.filter(
            IdAsesor=asesor_autenticado,
            descripcion__in=["AGENDADO", "VOLVER A LLAMAR"]
        ).values('dni_cliente').annotate(max_fecha=Max('fecha_hora'))

        agendados = Cliente.objects.filter(
            IdAsesor=asesor_autenticado,
            descripcion__in=["AGENDADO", "VOLVER A LLAMAR"],
            fecha_hora__in=subquery.values('max_fecha')
        ) if subquery.exists() else []

        context = {
            "ventas_por_dia": ventas_por_dia_dict,  # Datos concretos para el template
            "asesor": asesor_autenticado,
            "agendados": agendados,  # Se pasan los agendados al contexto
        }

        if request.method == "POST":
            # Procesa la lógica del POST aquí (por ejemplo, actualizar o crear registros)
        ...
        # Obtener los asesores que ya registraron asistencia hoy
        asesores_con_asistencia = Asistencia.objects.filter(fecha=fecha_actual).values_list('asesor_id', flat=True)

        asesores_sin_asistencia = asesores_en_gestion.exclude(IdAsesor__in=asesores_con_asistencia)

        # Esto es para ventas y se muestre en el grafico de supervisor 
        ventas_hoy = Venta.objects.filter(fecha_hora__date=fecha_actual, IdAsesor__in=asesores_en_gestion)

        #Agrupar ventas por día
        ventas_por_dia = ventas_hoy.values(fecha=F('fecha_hora__date')).annotate(count=Count('IdVenta'))

        #Agrupar ventas por asesor
        ventas_por_asesor = ventas_hoy.values(nombre=F('IdAsesor__nombre_apellido')).annotate(count=Count('IdVenta'))

        # Diccionarios para enviar a la plantilla
        ventas_por_dia_dict = {venta['fecha'].strftime('%Y-%m-%d'): venta['count'] for venta in ventas_por_dia}
        ventas_por_asesor_dict = {venta['nombre']: venta['count'] for venta in ventas_por_asesor}
        ...
        context = {
            ...
            'asesores': asesores_sin_asistencia,  # Solo los asesores sin asistencia registrada
            'clientes': clientes,  # Aquí pasas los clientes al contexto
            'tipificaciones_data': tipificaciones_data,
            'ventas_por_dia': json.dumps(ventas_por_dia_dict),  # Convertir a JSON
            'ventas_por_asesor': json.dumps(ventas_por_asesor_dict),  # Convertir a JSON
        }
